...
describe("คำสั่งยาว — สินค้า + ตัวเลือกในประโยคเดียว", () => {
  it("พูดตัวเลือกบังคับ + ตัวเลือกเสริมมาพร้อมกัน → เพิ่มได้เลย", () => {
    const resolution = add("เพิ่มอเมริกาโน่ร้อนคั่วเข้ม");
    expect(resolution.status).toBe("applied");
    expect(optionNames(resolution)).toEqual(["คั่วเข้ม", "ร้อน"]);
  });

  it("ตัวเลือกบังคับที่ 'มีค่าเริ่มต้น' อยู่แล้ว → เพิ่มได้เลย ไม่ต้องเด้งหน้าต่าง", () => {
    // "ประเภท" บังคับแต่ตั้งค่าเริ่มต้น "เย็น" ไว้ → ใช้ค่าเริ่มต้นได้ทันที
